...
The {\tt chipsort\_small} function achieved a significant speed-up of over 3 times compared to Insertion sort or the Julia standard function, and twice over {\tt chipsort!}. Even though this in-register function was clearly faster, this first experiment already demonstrates the potential of {\tt chipsort!}, which presented a speedup of at least 46\% compared to the other alternatives.
...
%
We have presented ChipSort, a Julia module for SIMD and cache-aware sorting. The main sorting method utilized in ChipSort is Combsort finalized with Insertion sort. This method is based on previous proposals for SIMD sorting~\cite{DBLP:conf/IEEEpact/InoueMKN07,DBLP:journals/pvldb/InoueT15} and other practical sorting methods~\cite{INCERPI198737,musser1997introspective}. This method can be implemented as a serial program that can be vectorized by the Julia compiler and LLVM, resulting in a sorting function up to twice as fast as the Julia standard library. This performance holds for at least a million 32-bit elements in a conventional computer.

Other techniques available in ChipSort are sorting networks, bitonic merge networks, in-place matrix transpose and a multi-way merge tree. Vectorized in-register sorting, although limited to small powers of 2, proved to deliver speedups of up to 3 times relative to the standard library. For large arrays there have been limited results, only narrowly surpassing the main ChipSort method in few occasions. Merge-based techniques are expected to be more advantageous when sorting large registers, minimizing memory access.

The main ChipSort method relies heavily on compiler optimizations, while other methods utilize explicit vectorization trough SIMD.jl. ChipSort makes extensive use of metaprogramming, in epecial Julia generated functions. This allows functions like sorting networks to be implemented in an abstract and generic way, based on representations of the networks as data structures used to assemble the final code by a high-level program. This contrasts with similar implementations in other libraries where networks are implemented straight as final functions. Julia also has the opportunity of performing many other optimizations since the library is implemented in Julia with abstract methods that can be specialized.

The best result obtained in ChipSort is a simple serial function that gets vectorized. 
